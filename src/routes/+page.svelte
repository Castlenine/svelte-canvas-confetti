...
<script lang="ts">
	...
	let activeTab: TabKey = 'falling';

	let particleCount = 50;
	let styles: ParticleStyleEntry[] = [];
	let styleMetadata: {
	...
	let emojiStyles: ParticleStyleEntry[] = [];
	...
	let burstOrigin: Position = [0, 0];

	let cannonOrigin: Position = [0, 0];
	let cannonForce = 15;
	let cannonAngle = 295;
	let cannonSpread = 60;

	let sparkleDuration = 3;
	let sparkleSpeed = 2;
	let sparkleArea: Position = [0, 0];
	let sparkleAreaOrigin: Position = [0, 0];

	let fireworkCount = 3;
	let fireworkBurstForce = 12;
	let fireworkLaunchForce = 25;
	let fireworkStaggerDelay = 400;
	let fireworkRocketColor = '#f5f5f5';

	let emojiOrigin: Position = [0, 0];

	let particleGravity: number | null = null;
	let particleOpacity: number | null = null;
	let particleSway: number | null = null;
	let particleRotation: number | null = null;
	let particleDelay: number | null = null;

	let fallingInstances: ConfettiInstance[] = [];
	let burstInstances: (ConfettiInstance & { origin: Position })[] = [];
	let cannonInstances: CannonInstance[] = [];
	...
	let parachuteInstances: { id: number; particleCount: number }[] = [];
	let emojiBurstInstances: (ConfettiInstance & { origin: Position })[] = [];
	let counter = 0;

	$: IS_MAIN_EFFECT = activeTab !== 'parachutes' && activeTab !== 'emoji';

	$: IS_FIRE_DISABLED =
		activeTab === 'parachutes'
			? parachuteImg == null
			: activeTab === 'emoji'
				? emojiStyles.length === 0
				: styles.length === 0;

	$: FIRE_LABEL = findFireLabel(activeTab);
...
<div class="page">
				...
				<div class="particle-props-grid">
					<label class="particle-prop-item">
						...
						<input
							...
							type="text"
							placeholder="default"
							on:change={handleParticleGravityChange} />
					</label>
					{#if activeTab !== 'sparkle'}
						<label class="particle-prop-item">
							...
							<input
								...
								type="text"
								placeholder="default"
								on:change={handleParticleOpacityChange} />
						</label>
					{/if}
					<label class="particle-prop-item">
						...
						<input
							...
							type="text"
							placeholder="default"
							on:change={handleParticleSwayChange} />
					</label>
					<label class="particle-prop-item">
						...
						<input
							...
							type="text"
							placeholder="default"
							on:change={handleParticleRotationChange} />
					</label>
					<label class="particle-prop-item">
						...
						<input
							...
							type="text"
							placeholder="default"
							on:change={handleParticleDelayChange} />
					</label>
				</div>
			</div>
		{/if}

		<button class="fire-button" type="button" disabled={IS_FIRE_DISABLED} on:click={handleFire}>
			{FIRE_LABEL}
		</button>
	</section>
	...
	{#each fallingInstances as instance (instance.id)}
		<FallingConfetti
			...
			on:completed={() => handleFallingCompleted(instance.id)}
	...
	{#each burstInstances as instance (instance.id)}
		<ConfettiBurst
			...
			particleCount={instance.particleCount}
			styles={instance.styles}
			on:completed={() => handleBurstCompleted(instance.id)}
			onCreate={instance.onCreate} />
	{/each}
	...
	{#each cannonInstances as instance (instance.id)}
		<ConfettiCannon
			...
			styles={instance.styles}
			on:completed={() => handleCannonCompleted(instance.id)}
			onCreate={instance.onCreate} />
	{/each}
	...
	{#each sparkleInstances as instance (instance.id)}
		<ConfettiSparkle
			...
			styles={instance.styles}
			on:completed={() => handleSparkleCompleted(instance.id)}
			onCreate={instance.onCreate} />
	{/each}
	...
	{#each fireworkInstances as instance (instance.id)}
		<ConfettiFireworks
			...
			styles={instance.styles}
			on:completed={() => handleFireworkCompleted(instance.id)}
			onCreate={instance.onCreate} />
	{/each}
	...
	{#if parachuteImg}
		{#each parachuteInstances as { id, particleCount } (id)}
			<FallingConfetti
				{particleCount}
				styles={[parachuteImg]}
				on:completed={() => handleParachuteCompleted(id)}
	...
	{#each emojiBurstInstances as instance (instance.id)}
		<ConfettiBurst
			...
			particleCount={instance.particleCount}
			styles={instance.styles}
			on:completed={() => handleEmojiBurstCompleted(instance.id)}
			onCreate={instance.onCreate} />
	{/each}
