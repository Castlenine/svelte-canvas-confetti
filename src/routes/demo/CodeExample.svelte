...
<script lang="ts">
	...
	export let activeTab: Props['activeTab'];
	export let particleCount: number;
	export let styleMetadata: readonly StyleMeta[];
	export let emojiStyleMetadata: readonly EmojiStyleMeta[];
	export let burstOrigin: Position;
	export let sparkleDuration: number;
	export let sparkleSpeed: number;
	export let sparkleArea: Position;
	export let sparkleAreaOrigin: Position;
	export let fireworkCount: number;
	export let fireworkBurstForce: number;
	export let fireworkLaunchForce: number;
	export let fireworkStaggerDelay: number;
	export let fireworkRocketColor: string;
	export let emojiOrigin: Position;
	export let particleGravity: number | null;
	export let particleOpacity: number | null;
	export let particleSway: number | null;
	export let particleRotation: number | null;
	export let particleDelay: number | null;
	export let cannonOrigin: Position;
	export let cannonForce: number;
	export let cannonAngle: number;
	export let cannonSpread: number;
	...
	// prettier-ignore
	$: code = generateCode(activeTab, particleCount, styleMetadata, emojiStyleMetadata, burstOrigin,
		sparkleDuration, sparkleSpeed, sparkleArea, sparkleAreaOrigin,
		fireworkCount, fireworkBurstForce, fireworkLaunchForce, fireworkStaggerDelay, fireworkRocketColor,
		emojiOrigin, cannonOrigin, cannonForce, cannonAngle, cannonSpread,
		particleGravity, particleOpacity, particleSway, particleRotation, particleDelay);
	...
	function generateCode(..._dependencies: unknown[]): string {
		switch (activeTab) {
			case 'falling':
...
<div class="code-example">
	<div class="code-block">
		<button class="copy-button" class:is-copied={isCopied} type="button" on:click={handleCopy}>
