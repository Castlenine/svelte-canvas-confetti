...
@prop styles {ParticleStyleEntry[]} bind: - Active particle style entries array synced to parent.
@prop styleMetadata {readonly StyleEntryMeta[]} bind: - Per-entry metadata for code generation (type, label, fontSize, textColor, w, h).
...
<script lang="ts">
	...
	export let styles: ParticleStyleEntry[];
	export let styleMetadata: StyleEntryMeta[] | undefined = undefined;
	export let mode: 'all' | 'colors' | 'emoji' = 'all';
	export let label = 'Styles';
	...
	$: IS_SHOW_COLORS = mode === 'all' || mode === 'colors';
	$: IS_SHOW_EMOJI = mode === 'all' || mode === 'emoji';
	$: IS_SHOW_IMAGES = mode === 'all';
	...
	{#if IS_SHOW_COLORS}
		<div class="section">
			...
			<div class="color-swatches">
				{#each DEFAULT_COLORS as color (color)}
					<button
						...
						type="button"
						title={color}
						on:click={() => handleToggleColor(color)}>
			...
			<div class="custom-color-row">
				...
				<button class="add-button" type="button" on:click={handleAddCustomColor}>Add</button>
			</div>
		</div>
	{/if}
	...
	{#if IS_SHOW_EMOJI}
		<div class="section">
			...
			<div class="emoji-presets">
				{#each PRESET_EMOJIS as emoji (emoji)}
					<button class="emoji-pill" type="button" on:click={() => handleAddEmoji(emoji)}>{emoji}</button>
				{/each}
		...
		<div class="section">
			...
			<div class="custom-emoji-row">
				...
				<button class="add-button" type="button" on:click={handleAddCustomEmoji}>Add</button>
			</div>
		</div>
	{/if}
					...
					<div class="chip chip-sized-row">
						...
						{#if entry.type === 'emoji'}
							<div class="size-inline">
								<input
									...
									step="1"
									title="Emoji size in pixels"
									on:change={(event) => handleEntryFontSizeChange(index, getInputValue(event))} /><span
						...
						{:else if entry.type === 'text'}
							...
							<input
								...
								type="color"
								title="Text color"
								on:change={(event) => handleEntryTextColorChange(index, getInputValue(event))} />
							<select
								class="chip-font-select"
								value={entry.fontFamily ?? 'sans-serif'}
								title="Font family"
								on:change={(event) => handleEntryFontFamilyChange(index, getSelectValue(event))}>
								{#each FONT_FAMILIES as font (font)}
									<option value={font}>{font}</option>
								{/each}
							</select>
						{:else}
							<div class="size-inline">
								<input
									...
									on:change={(event) => handleWidthChange(index, getInputValue(event))} /><span class="size-unit"
									>w</span>
							</div>
							<div class="size-inline">
								<input
									...
									on:change={(event) => handleHeightChange(index, getInputValue(event))} /><span class="size-unit"
									>h</span>
							</div>
						{/if}
						<button class="chip-remove" type="button" title="Remove" on:click={() => handleRemoveEntry(index)}
							>&times;</button>
					</div>
				{/each}
			</div>
